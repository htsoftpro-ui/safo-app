...
const navItems = [
  ...
  { name: 'الملف الشخصي', route: '/profile', icon: '👤' },
]
  ...
  <div class="min-h-screen flex">
    ...
    <aside class="w-64 bg-gray-900 text-white flex-shrink-0 flex flex-col" :class="{ 'hidden': !sidebarOpen }">
      <div class="p-4 border-b border-gray-700">
        <h1 class="text-xl font-bold">🏪 سافو</h1>
        <p class="text-sm text-gray-400 mt-1">لوحة المورد</p>
      </div>
      ...
      <div class="p-4 border-t border-gray-700">
        <div class="flex items-center gap-3 mb-3">
          ...
          <div>
            <p class="text-sm font-medium">{{ auth.user?.store_name || auth.user?.name }}</p>
    ...
    <div class="flex-1 flex flex-col">
      <header class="bg-white shadow-sm border-b px-6 py-3 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          <span class="text-sm text-gray-600">{{ auth.user?.store_name }}</span>
        </div>
      </header>
